...
class TripSearchViewModel {
    ...
    /// Pré-remplit le formulaire depuis un billet scanné
    func applyScannedTicket(_ ticket: ScannedTicket) {
        origin      = ticket.origin
        destination = ticket.destination
        date        = ticket.departureDate
        transport   = ticket.transportType
        // Déclenche la recherche d'horaires correspondants dans le cache
        Task { await searchRealRoutes() }
    }
